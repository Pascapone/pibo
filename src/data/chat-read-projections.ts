...
CREATE TABLE IF NOT EXISTS chat_read_backfill (id INTEGER PRIMARY KEY CHECK(id=1),cursor INTEGER NOT NULL DEFAULT 0,target INTEGER NOT NULL,paused INTEGER NOT NULL DEFAULT 0);
INSERT OR IGNORE INTO chat_read_backfill(id,target) SELECT 1,COALESCE(MAX(rowid),0) FROM chat_messages;
...
export class ChatReadProjectionStore {
 ...
 status() {
  const r=this.db.prepare("SELECT cursor,target,paused FROM chat_read_backfill WHERE id=1").get() as {cursor:number;target:number;paused:number};
  return {...r,paused:Boolean(r.paused),complete:r.cursor>=r.target};
 }
