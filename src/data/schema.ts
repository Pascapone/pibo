...
function rebuildRetiredScopeTables(db: DatabaseSync): void {
	const tablesToRebuild = retiredScopeTables.filter((table) => (
	...
	if (db.isTransaction) throw new Error("Pibo data schema migration requires an independent transaction");

	const foreignKeysEnabled = Number((db.prepare("PRAGMA foreign_keys").get() as { foreign_keys?: number }).foreign_keys ?? 0) === 1;
