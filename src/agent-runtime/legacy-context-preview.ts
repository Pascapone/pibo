...
export function buildLegacyContextPreview(profile: InitialSessionContext, cwd: string, piboSessionId?: string, piboRoomId?: string): PiboContextBuildSnapshot {
	...
	for (const pkg of profile.piPackages) add({ id: `retired-pi-package:${pkg.id}`, kind: "metadata", title: pkg.id, source: "legacy", state: "disabled", notes: ["Executable Pi package discovery retired; explicit Pibo plugin migration required."] });
