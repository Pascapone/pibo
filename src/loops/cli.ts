...
function formatLoopJobLine(job: PiboLoopJob): string { return `${job.id}\t${job.mode}\t${job.enabled ? 'running' : 'stopped'}\t${job.state.runningAt ? 'active' : '-'}\tresources=${formatLoopResourceSummary(job.resources)}\t${job.name}`; }
