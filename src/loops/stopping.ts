...
import type { PiboLoopFactReader, PiboLoopJob, PiboLoopRun, PiboLoopRunOutcome, PiboLoopStopAction, PiboLoopStopConditionDecision, PiboLoopStopConditionDefinition, PiboLoopStopConditionEvaluation, PiboLoopStopConditionInstance, PiboLoopStopConditionPhase, PiboLoopStopEvaluationSummary, PiboLoopStopPolicy } from './types.js';
...
export function getEffectiveLoopStopPolicy(job: PiboLoopJob): PiboLoopStopPolicy {
	return job.stopPolicy ?? getDefaultLoopStopPolicy(job);
}

export function createBuiltInLoopStopConditions(): PiboLoopStopConditionDefinition[] {
	return [
		{
		...
		},
		{
			...
			evaluate(context): PiboLoopStopConditionDecision {
...
function severity(action: PiboLoopStopAction): number {
	if (action === 'cancel-current-run') return 2;
	if (action === 'stop-after-run') return 1;
	return 0;
}
function highest(left: PiboLoopStopAction, right: PiboLoopStopAction): PiboLoopStopAction { return severity(right) > severity(left) ? right : left; }
function sanitizeDecision(value: unknown): PiboLoopStopConditionDecision {
...
export async function evaluateLoopStopPolicy(input: { job: PiboLoopJob; phase: PiboLoopStopConditionPhase; definitions: readonly PiboLoopStopConditionDefinition[]; facts: PiboLoopFactReader; run?: PiboLoopRun; outcome?: PiboLoopRunOutcome; now?: Date; signal?: AbortSignal }): Promise<{ evaluation: PiboLoopStopEvaluationSummary; conditionStates: Record<string, PiboJsonObject> }> {
	...
	const decisions: PiboLoopStopConditionEvaluation[] = [];
	const conditionStates: Record<string, PiboJsonObject> = { ...(input.job.state.conditionStates ?? {}) };
	let finalAction: PiboLoopStopAction = 'continue';
	for (const instance of policy.conditions) {
		...
		if (!definition) { decisions.push({ ...base, action: 'continue', skipped: true, error: `Unknown Loop stop condition type: ${instance.type}` }); continue; }
		...
		try {
			...
			const action: PiboLoopStopAction = failClosed ? 'stop-after-run' : 'continue';
	...
	if (policy.mode === 'all' && active.length > 0) {
		...
		finalAction = allStop ? active.reduce<PiboLoopStopAction>((action, decision) => highest(action, decision.action), 'stop-after-run') : 'continue';
	} else {
		finalAction = active.reduce<PiboLoopStopAction>((action, decision) => highest(action, decision.action), 'continue');
	}
