				...
				<div className="max-w-5xl mx-auto p-5 space-y-5">
					...
					<section className="grid grid-cols-[minmax(0,1fr)_minmax(280px,360px)] gap-4 max-[980px]:grid-cols-1">
						<Panel title="Job">
							...
							{draft.mode === "goal" ? <><Field label="Soft Token Budget"><input className={inputClass} type="number" min="1" value={draft.tokenBudget} onChange={(event) => setDraft({ ...draft, tokenBudget: event.target.value })} placeholder="Unbounded" /></Field><Field label="Pre-turn Token Reserve"><input className={inputClass} type="number" min="0" value={draft.tokenReserve} onChange={(event) => setDraft({ ...draft, tokenReserve: event.target.value })} placeholder="0" /></Field><div className="rounded-sm border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-xs text-slate-500">The budget is soft because usage arrives after each model response; the final turn can overshoot. Pibo starts another turn only while remaining tokens exceed the reserve.</div></> : null}
...
function runtimeSummary(job: PiboLoopJob): string { const parts = [job.mode, job.mode === "goal" ? job.state.goalStatus ?? (job.enabled ? "active" : "paused") : undefined, job.tokenBudget ? `soft ${job.state.tokensUsed ?? 0}/${job.tokenBudget}, reserve ${job.tokenReserve ?? 0}` : undefined, job.mode === "goal" ? `active ${formatDurationSeconds(goalActiveTimeSeconds(job))}` : undefined, job.modelOverride ? `${job.modelOverride.provider}/${job.modelOverride.id}` : undefined, job.thinkingLevel ? `thinking ${job.thinkingLevel}` : undefined, job.fastMode !== undefined ? job.fastMode ? "fast on" : "fast off" : undefined].filter(Boolean); return parts.length ? parts.join(" · ") : "default runtime"; }
