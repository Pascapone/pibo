...
type LoopJobBody = { mode?: unknown; name?: unknown; description?: unknown; enabled?: unknown; target?: unknown; profile?: unknown; prompt?: unknown; maxIterations?: unknown; tokenBudget?: unknown; stopPolicy?: unknown; modelOverride?: unknown; thinkingLevel?: unknown; fastMode?: unknown };
...
export async function handleChatLoopApiRequest(options: ChatLoopApiOptions): Promise<Response | undefined> {
	...
	if (apiPath === `${CHAT_WEB_API_PREFIX}/loops/jobs` && request.method === 'POST') {
		...
		if (mode === 'ralph' && tokenBudget !== undefined) throw new PiboWebHttpError('tokenBudget is only available for goal mode', 400);
	...
	if (request.method === 'PATCH') {
		...
		if ((patch.mode ?? existing.mode) === 'ralph' && patch.tokenBudget !== undefined && patch.tokenBudget !== null) throw new PiboWebHttpError('tokenBudget is only available for goal mode', 400);
