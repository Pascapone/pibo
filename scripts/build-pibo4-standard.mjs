...
if (standardSet.core !== "@pasko70/pibo" || standardSet.standard !== "@pasko70/pibo-standard") {
	throw new Error("Pibo Standard requires the Core and Standard package identities");
}
assertStandardPluginComposition(standardSet.plugins, defaults.standardPluginCoordinates());
			...
			: process.platform === "darwin" && process.arch === "arm64"
				...
				: process.platform === "win32" && process.arch === "x64"
					...
					: process.platform === "win32" && process.arch === "arm64"
						...
						: undefined;
if (!codexPlatformPackage) throw new Error("Pibo Standard does not support Codex Native on this build platform");
const codexPackageRoot = dirname(require.resolve("@openai/codex/package.json"));
const codexPlatformPackageRoot = dirname(require.resolve(codexPlatformPackage + "/package.json"));
...
await writeFile(join(standardRoot, "package.json"), `${JSON.stringify({
	...
	dependencies: Object.fromEntries([[standardSet.core, releaseVersion], ...standardSet.plugins.map((entry) => [entry.package, entry.version]), ["@openai/codex", "0.153.2"]]),
	optionalDependencies: { [codexPlatformPackage]: "npm:@openai/codex@" + JSON.parse(await readFile(join(codexPlatformPackageRoot, "package.json"), "utf8")).version },
	bundledDependencies: [...dependencyNames, "@openai/codex", codexPlatformPackage],
	engines: { node: ">=24" },
}, null, 2)}\n`);
