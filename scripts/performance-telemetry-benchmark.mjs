...
try {
 ...
 for(const session of targets)recorder.recordOutput({type:'message_started',piboSessionId:session.id,eventId:`turn-${session.id}`,text:'synthetic',source:'user'},{session});
 ...
 while(performance.now()-start<seconds*1000){
  ...
  for(let i=0;outputs<Math.floor(elapsed*100)&&i<25;i++,outputs++){const session=targets[outputs%10],id=`tool-${outputs}`;launch('output',()=>storage.ingestOutput({session,roomId:session.metadata.chatRoomId,event:{type:'tool_execution_finished',piboSessionId:session.id,eventId:`turn-${session.id}`,toolCallId:id,toolName:'benchmark',isError:false,result:{content:[{type:'text',text}]}}}));}
 ...
 const result={at:new Date().toISOString(),scope:'Synthetic storage-worker/telemetry contention. Diagnostic SQL statement executions classify UPSERT by its leading INSERT verb; these are not physical row-insert counts. WAL size is observed file length, not fsync count or cumulative WAL writes. No provider, browser, output retry orchestration or full integrated capacity claim.',requestedSeconds:seconds,elapsedMs:performance.now()-start,payloadBytes:size,sessions:10,generated:{deltas,outputs,admissions},completedCommands,simulatedImmediateCommandCompletion:true,admissionMs:stats(timings.admission),outputMs:stats(timings.output),failures,producerRefused,peakPending,peakTelemetryBytes,telemetry:writer.status(),loopMs:{p99:loop.percentile(99)/1e6,max:loop.max/1e6},rssBytes:{first:rss[0],last:rss.at(-1),max:Math.max(...rss)}};
