...
- Candidate next runtime seam in `runtime/index.ts`: agent node dispatch. It is now the remaining large dispatch block in the runtime entry point, and existing `runtime-agent-node.test.ts`, `runtime-prompt-workflows.test.ts`, `runtime-mixed-node-workflow.test.ts`, one-node workflow tests, and Pibo routing tests cover profile resolution, prompt building, Pibo executor invocation, output validation, metadata/session linkage, and persistence-adjacent behavior.
