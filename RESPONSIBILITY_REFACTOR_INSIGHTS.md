...
- `packages/workflows/src/store/index.ts` remains a high-responsibility module (~1,173 LOC) combining public store interfaces/list filters, one large `SqliteWorkflowRunStore` class, and write-side JSON serialization helpers.
...
- Next likely safe store extraction: move public store interfaces and list filter types into a `packages/workflows/src/store/contracts.ts` module re-exported by the store entry point, then have `SqliteWorkflowRunStore` import those contracts internally. This should reduce entry-point overload without behavior drift, but inspect downstream imports before moving exported names.
