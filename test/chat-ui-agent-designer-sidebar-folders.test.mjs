...
const coreWorkspaceSource = readFileSync("src/apps/chat-ui/src/core-workspace-view.tsx", "utf8");
...
test("Agent Designer uses the standard responsive sidebar contract", () => {
	...
	assert.match(coreWorkspaceSource, /initialAgentFolders=\{bootstrap\.agentFolders\}/);
	assert.match(coreWorkspaceSource, /surface="tab"/);
...
test("Agent picker exposes folder creation, rename, assignment, and empty-folder deletion", () => {
	...
	assert.match(sidebarSource, /aria-label=\{`\$\{collapsed \? "Expand" : "Collapse"\} \$\{label\}`\}/);
