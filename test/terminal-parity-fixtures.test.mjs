...
test("shared fixture asserts row order, prefixes, token tones, card descriptors, and redaction", () => {
	...
	assert.deepEqual(rows.map((row) => [row.id, row.kind, row.status]).slice(0, 17), [
		...
		["terminal:delegation:ps_linked_explorer", "agent.delegation", "done"],
		["node-agent-async", "agent.async", "done"],
		["terminal:run:run_fixture_typecheck", "yielded.run", "running"],
...
test("web-derived long-output fixture covers every preview source without character truncation", () => {
	...
	assert.deepEqual(rows.map((row) => [row.id, row.kind]), [
		...
		["terminal:run:run_web_derived_long_output", "yielded.run"],
