...
test("Goal active time adds only the current run to persisted active time", async () => {
	const script = `
		...
		console.log(JSON.stringify([
			...
			goalActiveTimeSeconds(goal({ activeTimeSeconds: 125, activeTimeRunningAt: "2026-08-10T10:04:00.000Z", runningAt: "2026-08-10T10:03:00.000Z" }), nowMs),
			goalActiveTimeSeconds(goal({ activeTimeSeconds: 125, activeTimeRunningAt: null, runningAt: "2026-08-10T10:04:00.000Z" }), nowMs),
	...
	assert.deepEqual(JSON.parse(stdout), [242, 242, 125, 125, 7, 125]);
});
