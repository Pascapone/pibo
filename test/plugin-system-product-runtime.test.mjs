...
test('product runtime starts persisted plugins and publishes one manager/host/session-plan service', async t => {
  ...
  for (const pluginId of ['pibo.preview', 'pibo.cron', 'pibo.workflows', 'pibo.code-runtime', 'pibo.file-editing', 'pibo.web-search', 'pibo.browser-tools', 'pibo.codex-compat', 'pibo.run-control', 'pibo.goal-control', 'pibo.runtime-pi', 'pibo.runtime-codex-native', 'pibo.runtime-muse-native', 'pibo.runtime-omp']) {
  ...
  assert.deepEqual(projection.getAgentRuntimeInstanceIds().sort(), ['codex-native', 'muse-native', 'omp-native', 'pi']);
  assert.deepEqual(projection.getProfileNames(), ['base', 'pibo-gateway-producer', 'codex-native', 'muse-native', 'orp']);
