...
test("default tool bundles expose their live count and use single-click child details", async () => {
	...
	assert.match(source, /data-pibo-tool-call-group-count="true"[\s\S]*?>\{toolCallCount\}<\/span>[\s\S]*?<TerminalRowContent/, "the live count must render before the current tool icon");
