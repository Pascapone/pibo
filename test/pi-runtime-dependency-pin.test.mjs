...
test("Pi runtime packages use one exact compatible version", async () => {
	...
	assert.deepEqual(versions, piPackages.map(() => "0.85.0"));

	for (const packageName of piPackages) {
		assert.equal(packageLock.packages[""].dependencies[packageName], "0.85.0");
		...
		assert.deepEqual(lockedCopies, lockedCopies.map(() => "0.85.0"));
	}
});
