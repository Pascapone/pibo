...
test("pibo debug pty help is discoverable", async () => {
	...
	assert.match(root.stdout, /pty\s+Run and inspect interactive commands under a PTY/);
	assert.match(root.stdout, /pibo debug pty run -- pibo --help/);

	const pty = await execFileAsync("node", [cliPath, "debug", "pty", "--help"]);
	assert.match(pty.stdout, /pibo debug pty - run and inspect interactive commands under a pseudo-terminal/);
