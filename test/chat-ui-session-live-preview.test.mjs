...
test("Preview package scopes lifecycle state and fullscreen to the selected Pibo Session", () => {
	const pane = readFileSync("src/apps/chat-ui/src/session-trace-pane.tsx", "utf8");
	const previewView = readFileSync("src/apps/chat-ui/src/plugins/preview-view.tsx", "utf8");
	...
	assert.match(previewView, /subscribeSessionLivePreviewEvents/);
	assert.match(previewView, /preview\.piboSessionId !== props\.piboSessionId/);
	assert.match(previewView, /fixed inset-0 z-\[100\]/);
	assert.doesNotMatch(pane, /openDesktopToolRef\.current\?\.\("preview"\)/);
	assert.doesNotMatch(pane, /id: "preview"/, "Preview is opened through the plugin workspace, not a duplicate core tab");
	assert.match(pane, /<SessionLivePreviewPanel/);
	assert.match(pane, /<PreviewFullscreenTopBar/);
	assert.match(previewView, /onEnterFullscreen=\{\(\) => setFullscreen\(true\)\}/);
	assert.match(previewView, /onExitFullscreen=\{\(\) => setFullscreen\(false\)\}/);
