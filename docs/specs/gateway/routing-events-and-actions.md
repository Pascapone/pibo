...
- Registered actions: `status`, `compact`, `runtime.approval.respond`, `runtime.user_input.respond`, `session_id`, `clear_queue`, `abort`, `kill`, `kill_all`, `dispose`, `thinking`, `fast_mode`, `sandbox`, `session.current`, `session.list`, `session.fork_candidates`, `session.fork`, `session.clone`, `session.tree`, `session.tree_navigate`, `session.switch`, `login`, `model`, `login.start`, `login.complete`, `login.apikey`, `login.cancel`, `login.status`, `logout`.
