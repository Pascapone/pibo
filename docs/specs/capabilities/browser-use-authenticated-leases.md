...
- Lease registry locking, slot naming, TTLs, holder labels, maximum active slot limits, and JSON/text output.
...
- Acquired leases include app, holder, session name, profile name, user-data dir, status, timestamps, and expiry.
...
- WHEN an agent runs `pibo tools browser-use lease acquire --app pibo-chat --holder agent-a --template-dir <template>`
...
`lease list` auto-reaps stale leases, then prints rows with id, state, holder, session name, and user-data dir. JSON output includes `expired` and `processAlive`. `lease release <id>` kills the CDP pid for the lease session when present, marks the lease released, and optionally deletes the slot profile.
...
- Lease acquire should expose enough state for agents to distinguish auth slot identity, browser-pool lease identity, CDP URL, holder, expiry, and cleanup status.
...
- App, holder, and profile labels can contain unsafe characters; slot ids MUST sanitize app names while retaining the display holder in the lease record.
...
- `pibo tools browser-use lease list` reports the active lease id, holder, session name, and user-data directory. Verified by `test/tools-cli.test.mjs`.
