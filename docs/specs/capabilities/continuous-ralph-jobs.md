...
`emitMessageAndWait()` waits without a time limit by default so an active agent can complete long-running work. Hosts and tests may set `runTimeoutMs` explicitly to opt into a bounded run. A correlated `session_error` terminates the run immediately instead of relying on a timeout. `recoverInterruptedRuns()` marks old running jobs as failed after a cutoff and records `Ralph run was interrupted by gateway restart` with reason `interrupted`.
...
- `test/ralph-run-timeout.test.mjs` verifies that Ralph runs have no timeout by default, correlated session errors remain terminal, and explicitly configured run timeouts still work.
