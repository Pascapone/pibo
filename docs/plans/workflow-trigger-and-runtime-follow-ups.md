...
sources:
  ...
  - id: "SPC-ORCH-006"
    resource: "/specs/orchestration/workflow-catalog-and-project-execution.md"
    title: "Workflow Catalog, Publishing, and Project Execution"
    relation: "shipped catalog and Project execution facts"
  - id: "SPC-WEB-008"
...
links:
  ...
  - "/specs/orchestration/workflow-catalog-and-project-execution.md"
...
The canonical specifications own current workflow behavior. Pibo already has workflow IR, validation and registry helpers, versioned catalog/draft/publish/archive surfaces, an editor, runtime/store components, a bounded manual text-trigger slice with deterministic fan-out, and Project configuration plus initial run records. The current manual slice explicitly rejects unsupported joins, Project start does not prove full graph advancement, and no general restart-resumption or external webhook/Cron trigger is evidenced.
...
- Decide and normalize first-class trigger-node and trigger-envelope semantics across IR, editor, Project, and later provider starts.
...
- Connect Project and editor starts to the owned graph executor without duplicating execution in the Web layer.
...
- Claim Project full-graph execution, restart recovery, webhook/Cron triggers, or live provider behavior before dedicated gates pass.
...
2. **Start envelope:** define workflow/version, trigger kind, typed input, idempotency key, project/session ownership, actor, capability snapshot, and requested execution policy.
...
- Route editor test and Project start through one orchestration-owned start contract.
...
Dependency: current Project idempotency and immutable configuration/version snapshots must remain intact.
...
- Project current and historical facts into Project and workflow views without making projections authoritative.
...
| Manual start | Editor and Project use one envelope; valid manual input starts once and invalid input cannot create a run. | Root API/service plus package tests. |
...
| Project | Immutable snapshots and one-run start semantics remain intact while graph advancement moves to orchestration ownership. | Project service and integration tests. |
