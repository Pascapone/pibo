...
| AP08 | in progress | PLG-UX-002 is implemented locally at `202cd2e3` and `a32b3466`: the selected Session owns the live tabset, inactive panels remain mounted, guarded Refresh/Close is generic, dirty/conflicted/in-flight controllers survive bounded cache pressure, and immediate optimistic rename cannot retarget newer navigation after delayed title persistence. Focused automated, build/typecheck, desktop/mobile delayed-POST and delayed-PATCH evidence is green. An earlier headful Agent Designer run confirms the shared callback; exact-candidate delayed-POST timing through that entrypoint remains an evidence gap. Final package acceptance still inherits AP19. |
...
| AP19 | blocked | Typechecks, the earlier clean stepwise build, package-content tests and desktop/mobile headful evidence are complete. After the automatic-upgrade correction, 35 migration/session/Gateway/UI/source-audit regressions pass; the earlier 113-test aggregate predates that correction and is not relabeled as current-candidate evidence. The later Session workspace correction has a committed source candidate, 64 focused passing tests, a passing Chat UI build/typecheck and a commit-backed lifecycle specification. Final closure still needs the current complete suite, Pibo2 real-provider Pi/Codex/OMP evidence, remaining specification reconciliation and complete acceptance-matrix evidence. |
...
- Baseline for the PLG-UX-002 follow-up: `202cd2e394e06da11a2c36b9be71ccd4649a09b9`. Immediate optimistic rename and delayed-title-persistence ownership fix: `a32b34661c8feab3ac270d21655cc95cab77315a`, validated in Docker `pibo-dev-plugin-system-rebuild` under `/workspace`. No push, PR, deployment or release followed.
...
The public workspace check preserved the same Settings panel and child DOM while switching to Preview and back; Refresh replaced only the Settings child. Session `ps_d1b8e0e7-e822-410f-ab8a-6c761e895855` restored Settings + Preview after switching Sessions. Newly created `ps_2c5fd57f-dc98-40c7-ba6d-a9305cd801e6` had no desktop tabs; returning restored the earlier pair and loaded Settings successfully. Evidence: `/tmp/pibo2-workspace-accept.json`, `/tmp/pibo2-workspace-desktop-ready.png`. Disposable acceptance Sessions were created without model turns or changes to user Agent profiles. No full suite, push, PR, merge, release or publication was performed.
