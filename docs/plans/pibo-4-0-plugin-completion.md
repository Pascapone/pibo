...
Stand 2026-09-15: `npm run pibo4:packages` erzeugt Minimal-Core, Standardkomposition, das getrennte Cutover-Werkzeug und 20 separat packbare Artefakte. `test/pibo4-packed-distribution.test.mjs` startet Minimal-Core aus einem Tarball ohne Quellcheckout und ohne Plugins, installiert Preview aus einem zweiten Tarball, packt alle Plugins sowie Standard/Cutover getrennt und prüft den physischen Ausschluss konkreter Runtime-/Run-/Delegation-Symbole. `test/pibo4-cutover.test.mjs` beginnt zusätzlich beim tatsächlichen gepackten `@pasko70/pibo@3.6.2`, schreibt vor dem Ersatz einen inhaltshashgebundenen Plan und aktiviert anschließend nur die dort als aktiv abgebildeten Artefakte. Der fokussierte Releasegrenzen-Commit markiert den breiten Root-Workspace mit `private: true` und sperrt den direkten Publish durch `prepublishOnly`; `scripts/release.mjs` publiziert ausschließlich `dist/pibo4-core-package`, `dist/pibo4-cutover-package`, die 20 Verzeichnisse unter `dist/pibo4-artifacts` und zuletzt `dist/pibo4-standard-package`. Der hermetische Release-Test zeichnet diese Aufrufe auf und verbietet den nackten Root-Publish. `/tmp/pibo4-f06-package-boundaries.md` und `/tmp/pibo4-f07-cutover.md` enthalten die früheren Paket-, Hash- und Cutovernachweise; der F10-Paketreview ergänzt die Releasepfadevidenz.
