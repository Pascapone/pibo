...
| D20 | Das Erstellen einer Session verwendet ausschließlich die Client-Navigation der laufenden Chat-App. Die neue Session wird aktiviert und ihr Inline-Rename-Feld fokussiert, ohne Document-/PWA-Reload. |
...
| D22 | Der sichtbare Desktop-Workspace ist Teil des revisionsgesicherten PluginStore-Tabsets seiner Pibo Session. Neue Sessions beginnen ohne echte Tabs; Reihenfolge, aktiver Tab und unterstützter View-Zustand werden weder Room- noch Browser-global geteilt. Ein begrenzter Controller-Cache darf nur verlustfrei verwerfbare Einträge entfernen: lokale Entwürfe, CAS-Konflikte und laufende Reads/Writes bleiben bis Speicherung oder ausdrücklicher Recovery erhalten, während Browser-Hosts und React-Panels beim Sessionwechsel weiterhin entsorgt werden. |
...
**Clientseitige Session-Erstellung:** Jede produktive „New Session“-Aktion innerhalb einer Room — einschließlich Agent Designer — verwendet denselben App-eigenen Create-/Selection-Flow. Nach erfolgreichem POST ersetzt die reale Session die optimistische Zeile, die Route wird über den Client-Router aktualisiert und das Inline-Rename-Feld der neuen Session erhält Fokus und Textauswahl. `location.assign`, `location.href`, Formularnavigation und Service-Worker-/Document-Reload sind in diesem Pfad unzulässig. Eine zwischenzeitliche bewusste Auswahl eines anderen Ziels gewinnt weiterhin gegen die späte Create-Antwort.
...
| A43 | In einer Room wird aus der Sessions-Sidebar und aus dem Agent Designer je eine neue Session erstellt. In beiden Fällen bleibt dieselbe Document-/PWA-Instanz aktiv, die URL wechselt per Client-Router zur realen Session-ID, die neue Zeile ist ausgewählt und ihr leeres Inline-Rename-Feld ist unmittelbar fokussiert. Eine während des POST gewählte andere Session bleibt ausgewählt. | AP08, AP17, AP19 |
...
| A45 | Session A und B besitzen unterschiedliche Tabs, Reihenfolge, aktiven Tab und View-Zustand; eine neue Session C ist leer. Schnelles A→B→A mit absichtlich vertauscht abgeschlossenen Reads und verzögerten Saves zeigt nie fremde Tabs und schreibt jede Antwort nur zu ihrem Owner. Nach einem fehlgeschlagenen Save bleibt der lokale Entwurf beim Besuch von mehr als acht weiteren Sessions im Controller-Cache erhalten; bereinigte Controller dürfen verdrängt werden, Live-Browser-Hosts der verlassenen Sessions jedoch nicht weiterlaufen. Browser-Reload stellt A beziehungsweise B exakt aus PluginStore wieder her; der globale v1-Key wird nicht als Laufzeit-Fallback gelesen oder beschrieben. | AP03, AP08, AP16, AP19 |
