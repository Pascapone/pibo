...
V4 ist V3 mit neuer Baseline und nachgezogener Remote-Wirklichkeit; alle Produktziele, Attachment-Zusagen (inkl. AT-01–AT-22) und Arbeitspakete gelten unverändert. Konkret: (1) Baseline `84101adc` statt Audit-Baseline; die Remote-Basis `a3472458` war schon in `ece5f18c` enthalten — keine Komplett-Neuintegration, sondern exakt ein Commit `175afcfa` (Observe-Spiegelung über dieselbe Query-Engine wie `pibo_agents_observe`, mit dokumentierten Remote-Abweichungen F1/F2/F7: 2 Src- + 2 Testdateien). (2) Tatsächliche Tests: 65/65 `test/remote-agent-*.test.mjs` grün, `tsc`-Emit fehlerfrei; vorbestehende Rotfunde (Chat-UI-TS2322, Doku-Tracecommit) sind Baseline, kein V4-Versäumnis. Die 65 Tests decken die F1–F7-Szenarien nicht bereits ab. (3) Remote Agent bleibt erhaltenes wichtiges Plugin (`pibo.remote-agent`); Erhaltungs-/Refactoringziel ist saubere Trennung vom Kern (heutige Kopplung an `subagents/tool` und Beta-Builder ist dokumentierter Befund). Remote ergänzt Cs Bündel — kein fünfter Dauer-Worker. (4) Redaktionell eindeutige Schreibgrenzen: B neutrale Verträge/Owner-Bindung, C Remote-/Transkriptions-/Tool-Verbraucher, D zentrale Web-/Attachment-Dateien, I Root/Builder; keine B+C-Doppelschreibrechte. (5) K07 ausschließlich Attachments; Unklarheiten zum Zukunftsvertrag vor dem jeweils benötigten Piloten klären (D1→I→C1 vor G1 bleibt). Details in Anhang R (§13) und im Remote-Plugin-Contract-Spec.
...
V4-Evidenz (Merge `84101adc`): 65/65 `test/remote-agent-*.test.mjs` grün (isoliert, EXIT 0), `tsc`-Emit fehlerfrei (EXIT 0). Separat zugeordneter fremder Schlusslauf (nicht versioniert, `merge-cleanup-r1/tests.md`): `npm test` @84101adc, 16:28–16:32Z, Exit 1 am pibo4:standard-22/21-Gate (vorbestehend, Paket-Delta 0), 0 Tests — Vollbuild/Vollsuite NICHT erfolgreich, kein eigener Nachlauf. Vorbefunde als Baseline, nicht als grün: `TS2322` in `chat-ui/composer-send.ts` (Typecheck EXIT 2, merge-unberührt) und Doku-Tracecommit `storage-maintenance.md`; keine Validatorabschwächung.
...
REMOTE / ERHALTENES PLUGIN: Remote Agent (`pibo.remote-agent`) bleibt erhaltenes wichtiges Plugin (Trennung vom Kern als Ziel, derzeit Kopplung an `subagents/tool` + Beta-Builder); Basis a3472458 war integriert, hinzu kam exakt die Observe-Spiegelung (Merge 84101adc). Raum-/Token-/Modul-/Auth-/Revocation-/Sandbox-/Datei-/Bash-Verträge unverändert lassen. Beobachtung über die gemeinsame Engine (Filter/cursorMode/afterSequence/order/limit, echte toolCallId/requestId/turnId); genutzte Observation-/Regex-/Helfer nicht löschen. Remote-MCP ist nicht der verworfene K07-MCP. Schreibgrenzen: B neutral, C Remote-Verbraucher, D Web/Attachments, I Root/Builder; keine Doppelrechte.
```
...
**V3-Arbeitsumfang dieses Updates war:** vorhandene Quellen gelesen, Plan und Briefings um K07 ergänzt; HTML-/Demo-Prüfung betrifft nur diese lokale Präsentation und ist keine Pibo-Abnahme. **Nicht ausgeführt:** Repositoryänderung, Workerstart, Produktbuild, neue Pibo-Tests, Publish oder Deployment. **V4-Umfang:** Remote-Merge + Tests + Doku (siehe §13/§14); keine Bloat-/K07-Umsetzung.
...
Vor Umsetzung gehört dieser Entwurf als Technical Design in `docs/plans/`. Erst die tatsächlich implementierten, getesteten Zusagen werden in die normativen Specs übernommen. Dateiname und OKF-Registrierung sind vor dem Schreiben durch A/I gegen die Projektregeln zu prüfen; dieses Gespräch hat noch keine Projektdatei angelegt. [V3-Stand; V4-Hinweis: Dieser Integrationsvorgang hat gezielt Projektdokumentationsdateien angelegt (V4-Plan/HTML, Spec, Entwurf, Glossar, Archiv); Repo-Codeänderung ist exakt der Remote-Merge 84101adc.]

V2 bleibt als bisheriger Plan erhalten, K07-MCP-Recherche als verworfene Alternative. Es gibt keine zwei gleichzeitig gültigen K07-Aufträge. Planstand 04 ist für diese Erweiterung die aktuelle Arbeitsgrundlage (V3-Original historisch archiviert).
...
V3-Prüfstand (historisch): In diesem Update wurden die vorhandenen Gesprächsdateien gelesen und die Planartefakte bearbeitet. **Keine Live-Repositoryprüfung, keine Projektcodeänderung, keine gestarteten Umsetzungs-Worker und keine neue Pibo-Produktabnahme.** Eine gegebenenfalls beigefügte HTML-Demo simuliert den Ablauf ausschließlich mit Beispieldaten im Browser; sie ist kein implementiertes Pibo-Feature. V4-Prüfgrenze: Code-Merge 84101adc verifiziert (0 Konflikte), 65/65 Remote-Tests isoliert grün, tsc-Emit fehlerfrei, Doku-Gates geprüft; keine neue Produktbrowserabnahme; Vollbuild/Vollsuite nur als fremder Schlusslauf (Exit 1, 22/21-Gate, 0 Tests, nicht versioniert), kein eigener Lauf.


**Prüfung der HTML-Präsentation (V3, historisch):** Raster mit 12, 6 und 3 Spalten, Overlay-Auswahl, Snapshotkopie, JSON-Vorschau und aktualisierte Worker-Dialoge wurden mit lokalen Beispieldaten im sichtbaren Chromium geprüft. Direkte Datei-/HTTP-Navigation war administrativ gesperrt; die Präsentation wurde offline geladen. Der Persistenz-Roundtrip nutzte einen Storage-Testadapter. Echte originbasierte Browserpersistenz, Binärressourcen und Pibo-Produktverhalten sind damit nicht geprüft. V4-HTML-Prüfung: JSON/JS-Syntax/Links/Demo-IDs verifiziert; keine headful Produktabnahme.
...
**Grenzen (konkret, B- + Controller-Review):** F1 Positions-Cursor können bei ms-Gleichstand Einträge verpassen/duplizieren (Wahrscheinlichkeit nicht gemessen; Charakterisierungstest als Ziel, stabiler Cursorvertrag später nötig). F2 `roles:[user]` ist remote ein Superset. F3: begrenzte AUSGABE ≠ begrenzte ARBEIT — alle Eventlog-Seiten + Observations + Payloads werden vor Query-Begrenzung geladen (`service.ts:591–602`); Lastprüfung mit großen Histories offen; B/C übernehmen später eine gemeinsame Datenzugriffsgrenze. F7: Auto-Cursor gehören Session + normalisiertem Query, NICHT dem Token (`observe.ts:60–61,223–228`; `service.ts:604–606`); zwei berechtigte Tokens können sich den Verbrauchsstand teilen — kein Cross-Room-Zugriff nachgewiesen, keine Token-Isolation zugesagt; Multi-Client-Fall als Folge-Testziel. F4-Brüche (neueste-zuerst, nur-Neues, Volltexte, Textformat) sind gewollt. Die 65/65 Tests decken F1–F7 NICHT ab. Kein neuer Server-/Auth-/Framework-Layer. K07 ausschließlich Attachments; Remote-MCP ≠ verworfener K07-MCP.

## 14. V4-Änderungsprotokoll (gegen V3, alle Editionen dokumentiert)

E1 Titel 03→04 · E2 Stand/Basis/Vorgänger · E3 Planänderung-04-Callout · E4 neuer §0 · E5 §2 Remote-Erhalt · E6 K01-Aufrufer · E7a–c K02-Engine/Nachweis/Nicht-Ziele · E8 K05-Nachweis · E9 K06-Nachweis · E10 K07-Abgrenzung · E10b K03-Nicht-Ziele · E11 je ein Remote-Bullet in I0/A1/B1/C1/D1/G1/B2/C2/D2/I2/Q3 · E12 §7-Schreibgrenzen · E13 §8-Evidenz · E14 §9-Remoteabsatz · E15 §11-Quellen M1/R1 · E16 12.13-Pilotregel · E17 dieser Anhang + Protokoll. E18 Controller-/Docs-Review-Nachzug: F1–F7 konkret (keine ≡-Parität, Installierbarkeit als Ziel), V3-Prüfstände historisch gerahmt, 4. OBSERVE-Test in Spec, Controller-Review als Review Record versioniert. E19 Fremder Schlusslauf zugeordnet (Exit 1, 22/21-Gate vorbestehend, 0 Tests; kein „Vollbuild nie versucht“). V3-Body sonst unverändert übernommen; V3-Konzept bleibt mit superseded-Metadaten archiviert.
