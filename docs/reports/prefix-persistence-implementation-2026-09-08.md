...
This is a partial implementation checkpoint, not completion of the plan or a release acceptance report. Work started from upstream/dev `d631e8f0e88b9e25024e706d79dec62d864f7456` in branch `persist-session-prefix`. No production rollout, controller gateway restart, Pibo2 acceptance, or code PR has occurred.
...
|---|---|---|
| Pi / `pi`, SDK 0.85.0, Codex Responses API | Actual loopback HTTP request recording using native ModelRuntime and native JSONL resume; changed base prompt and provider hooks; native Tool call/output; large history | Resource capsule and loader bypass; all provider APIs/optional modes; native first-dispatch skeleton durability; crash-safe compaction/history transitions; full router binding lifecycle; complete T01/T04/T08 matrix |
| Codex-native / `codex-native`, installed binary 0.153.2 | Local source inspection shows native base instructions persisted in rollout SessionMeta and used when resuming. Pibo supplies developer instructions/config again at thread start/resume. | Exact installed-version final request proof and capture/restore contract for native Tools/instructions/resources; configuration/resource freezing; native lifecycle integration. Local source HEAD is not proven to be the installed binary's exact revision. |
| OMP / `orp` | Local source `get_state` exposes systemPrompt and Tool wire schemas; Pibo thread controller retains native identity | Supported restoration of full native prompt and ordered Tools, final provider seam proof, resources, all lifecycle transitions and exact version conformance |
