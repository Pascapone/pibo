...
const lease=JSON.parse(await readFile('/tmp/room-edit-pibo2-lease.json','utf8'));
...
try{
 ...
 const setup=process.argv.includes('--resume')?JSON.parse(await readFile('/tmp/room-edit-pibo2-ids.json','utf8')):await c.evaluate(`(async()=>{const post=async(path,data)=>{const r=await fetch(path,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(data)});const body=await r.json();if(!r.ok)throw Error('Setup failed '+r.status);return body};const rooms=[];for(let i=0;i<2;i++)rooms.push((await post('/api/chat/rooms',{name:'Room edit Spark '+Date.now()+' '+i})).room.id);const sessions=[];for(const roomId of[rooms[0],rooms[0],rooms[1]])sessions.push((await post('/api/chat/sessions',{roomId,profile:'rt-pi-spark'})).session.id);return{rooms,sessions}})()`);
 Object.assign(state,setup);await writeFile('/tmp/room-edit-pibo2-ids.json',JSON.stringify(state,null,2));
 for(let i=0;i<state.sessions.length;i++){
  const id=state.sessions[i],room=state.rooms[i===2?1:0],marker='ROOM_EDIT_TARGET_'+id;
  ...
  state.turns.push({id,model,submit});await writeFile('/tmp/room-edit-pibo2-ids.json',JSON.stringify(state,null,2));
...
}finally{await writeFile('/tmp/room-edit-pibo2-ids.json',JSON.stringify(state,null,2));c.close()}
