...
const rooms=ids.rooms,sessions=[ids.sessions[0],ids.sessions[2]],results=process.argv.includes('--resume')?JSON.parse(await readFile('/tmp/room-pibo2-active-acceptance.json','utf8')).results.filter(r=>r.pass):[];
...
try{for(const width of [390])for(const scenario of scenarios.filter(s=>!s.failure&&((s.stage==='post'&&['other-room','browser-back'].includes(s.action))||(s.stage==='navigation'&&s.action==='next-room')))){
 if(results.some(r=>r.width===width&&JSON.stringify(r.scenario)===JSON.stringify(scenario)&&r.pass))continue;
 const result={width,scenario,at:new Date().toISOString()};results.push(result);result.sourceStatus=await c.evaluate(`(async()=>{const r=await fetch('/api/chat/status?piboSessionId=${sessions[0]}');const s=await r.json();return{processing:s.processing,queuedMessages:s.queuedMessages,activeModel:s.activeModel}})()`);assert.ok(result.sourceStatus.processing,'Active-work acceptance missed processing window');assert.equal(result.sourceStatus.queuedMessages,1);
 ...
 try{
  ...
  await writeFile('/tmp/room-pibo2-active-acceptance.json',JSON.stringify({ids,results},null,2));
 }
}
