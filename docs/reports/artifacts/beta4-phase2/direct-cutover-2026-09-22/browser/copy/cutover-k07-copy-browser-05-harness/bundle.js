...
function canonicalSendStructure(text2, attachments) {
  return {
    text: text2,
...
function canonicalSendValue(text2, attachments) {
  return toJsonText(canonicalSendStructure(text2, attachments), "Send value");
}
...
var CoreAttachmentDraftStore = class {
  ...
  freezeForSend(clientTxnId, text2) {
    ...
    const sendValue = canonicalSendValue(text2, attachments);
  ...
  commit(candidate, failureMessage) {
    ...
    try {
      this.storage.writeText(draftKey(this.sessionId), text2);
    } catch {
...
// src/apps/chat-ui/src/attachments/core-attachment-copy-state.ts
var AttachmentCopyConflict = class extends AttachmentDraftError {
...
function assertCopyRevision(state, expected) {
...
function checkEntry(value, typed) {
  if (!isPlainJsonObject(value) || !text(value.copyId) || !text(value.sourceSessionId) || !text(value.sourceDraftId) || !positive(value.sourceRevision) || !Object.hasOwn(value, "payload") || !Array.isArray(value.media) || typed && (!text(value.type) || !positive(value.schemaVersion))) throw invalidJson("Copy entries require source identity, revision, JSON, media and a provider type/schema.");
  assertJsonValue(value, "Copy entry");
  const ids = /* @__PURE__ */ new Set();
  for (const media of value.media) {
    if (!isPlainJsonObject(media) || !text(media.draftResourceId) || ids.has(media.draftResourceId) || !text(media.mimeType) || !Number.isSafeInteger(media.bytes) || media.bytes < 0) throw invalidJson("Copy media must have distinct identities, MIME types and non-negative safe byte lengths.");
    ids.add(media.draftResourceId);
  }
}
function captureCopyInput(value) {
...
function readCopyBufferState(value, ownerUserId) {
  if (value === void 0) return { revision: 0 };
  try {
    ...
    if (!Object.hasOwn(value, "formatVersion")) {
      if (!text(value.copyId) || !text(value.sourceSessionId) || !text(value.sourceDraftId) || !text(value.stagedAt) || !Array.isArray(value.media) || value.media.some((media) => !media || !text(media.draftResourceId))) throw invalidJson("Invalid legacy copy holder.");
      return { revision: 1, legacy: structuredClone(value) };
    }
    ...
    if (value.entry === null) return { revision: value.revision };
    ...
    return { revision: value.revision, entry: cloneJson(entry) };
  } catch {
    throw attachmentStorageFailed("Stored copy buffer is invalid; it was not changed.");
  }
}
function copyBufferRow(ownerUserId, current, entry) {
  return { formatVersion: 1, ownerUserId, revision: nextAttachmentRevision(current.revision), updatedAt: (/* @__PURE__ */ new Date()).toISOString(), entry };
}
function copyHolderDraftId(copyId) {
  return `pibo.copy-holder.${copyId}`;
}
function copyStateMedia(state) {
  return state.entry?.media ?? state.legacy?.media ?? [];
}
function requireScopedCopyBlob(blob, ownerUserId, sessionId, draftId, media, maxBytes) {
  if (!blob || blob.blobId !== media.draftResourceId || blob.ownerUserId !== ownerUserId || blob.sessionId !== sessionId || blob.draftId !== draftId || blob.mimeType !== media.mimeType || blob.size !== media.bytes || blob.size > maxBytes || !(blob.data instanceof ArrayBuffer) || blob.data.byteLength !== blob.size) {
...
function sameCopyContent(a, b) {
  return canonicalJson(a) === canonicalJson(b);
}
function rebaseCopyMedia(media, createId) {
...
async function readStoredBlobHolders(drafts, copies, ownerUserId) {
  ...
  const copy = readCopyBufferState(await attachmentIdbRequest(copies.get(ownerUserId)), ownerUserId);
...
async function openAttachmentStores(factory, ownerUserId, databaseName) {
  ...
  const releaseCopyBytes = async (previous, bytes, drafts, copies) => {
    ...
    for (const media of entry.media) if (!held.has(media.draftResourceId)) {
      ...
      if (blob?.ownerUserId === ownerUserId && blob.sessionId === entry.sourceSessionId && blob.draftId === copyHolderDraftId(entry.copyId)) await attachmentIdbRequest(bytes.delete(blob.blobId));
    }
  };
  const copy = {
    ownerUserId,
    stage: async (expectedRevision, value) => {
      ...
      return withStores([ATTACHMENT_COPY_STORE_NAME, ATTACHMENT_BLOB_STORE_NAME, ATTACHMENT_DRAFT_STORE_NAME], "readwrite", async ([copies, bytes, drafts]) => {
        const current = readCopyBufferState(await attachmentIdbRequest(copies.get(ownerUserId)), ownerUserId);
        assertCopyRevision(current, expectedRevision);
        const rawSource = await attachmentIdbRequest(drafts.get([ownerUserId, input.sourceSessionId]));
        ...
        if (!source || source.envelope.revision !== input.sourceRevision || source.envelope.type !== input.type || source.envelope.schemaVersion !== input.schemaVersion || !sameCopyContent(source.payload, input.payload) || !sameCopyContent(source.media ?? [], input.media)) throw new AttachmentDraftError({ code: "ATT_STALE_REVISION", message: "Copy source changed; reload before staging it.", retryable: false });
        const entry = { ...input, ownerUserId, stagedAt: (/* @__PURE__ */ new Date()).toISOString(), media: rebaseCopyMedia(input.media, createBlobId) };
        for (let index = 0; index < input.media.length; index++) {
          const media = input.media[index];
          const source2 = requireScopedCopyBlob(await attachmentIdbRequest(bytes.get(media.draftResourceId)), ownerUserId, input.sourceSessionId, input.sourceDraftId, media, ATTACHMENT_BLOB_MAX_BYTES);
          try {
            await attachmentIdbRequest(bytes.add({ ...source2, blobId: entry.media[index].draftResourceId, draftId: copyHolderDraftId(entry.copyId), createdAt: entry.stagedAt, data: source2.data.slice(0) }));
          } catch (error) {
        ...
        await attachmentIdbRequest(copies.put(row));
        await releaseCopyBytes(current, bytes, drafts, copies);
        return readCopyBufferState(row, ownerUserId);
      });
    },
    load: async () => withStores([ATTACHMENT_COPY_STORE_NAME], "readonly", async ([store]) => readCopyBufferState(await attachmentIdbRequest(store.get(ownerUserId)), ownerUserId)),
  ...
  return {
    ...
    clearOwner: async () => {
      await withStores([ATTACHMENT_BLOB_STORE_NAME, ATTACHMENT_COPY_STORE_NAME, ATTACHMENT_DRAFT_STORE_NAME], "readwrite", async ([blobStore, copyStore, draftStore]) => {
        ...
        const copy2 = readCopyBufferState(await attachmentIdbRequest(copyStore.get(ownerUserId)), ownerUserId);
        await attachmentIdbRequest(copyStore.put(copyBufferRow(ownerUserId, copy2, null)));
      });
    }
  };
}
...
async function openIndexedAttachmentDraft(options) {
  ...
  return {
    ...
    async pasteCopy(expectedRevision, expectedCopyRevision) {
      active();
      checkExpected(expectedRevision);
      checkExpected(expectedCopyRevision);
      const outcome = await attachmentIdbTransaction(db, [ATTACHMENT_DRAFT_STORE_NAME, ATTACHMENT_COPY_STORE_NAME, ATTACHMENT_BLOB_STORE_NAME], "readwrite", async ([drafts, copies, bytes]) => {
        active();
        const current = await readRow(drafts);
        compare(current, expectedRevision);
        const copy = readCopyBufferState(await attachmentIdbRequest(copies.get(ownerUserId)), ownerUserId);
        ...
        const next = transitionCoreAttachmentDraft(
        ...
        for (let index = 0; index < entry.media.length; index++) {
          ...
          const blob = requireScopedCopyBlob(
            await attachmentIdbRequest(bytes.get(sourceMedia.draftResourceId)),
            ownerUserId,
            entry.sourceSessionId,
            copyHolderDraftId(entry.copyId),
            sourceMedia,
            ATTACHMENT_BLOB_MAX_BYTES
          );
          staged.push({ ...blob, blobId: media[index].draftResourceId, sessionId, draftId: next.result, createdAt: (/* @__PURE__ */ new Date()).toISOString(), data: blob.data.slice(0) });
        }
        for (const blob of staged) {
          try {
            await attachmentIdbRequest(bytes.add(blob));
          } catch (error) {
...
function note(sessionId, text2 = "kept") {
  return { kind: "add", input: { sessionId, type: "pibo.core/note", schemaVersion: 1, payload: { text: text2 } } };
}
...
async function runCases() {
  ...
  await run("additive-v2-upgrade", async (name) => {
    ...
    try {
      ...
      equal((await stores.copy.clear(1)).revision, 2);
      equal(await stores.blobs.deleteBlob("old"), true);
    } finally {
      draft.close();
      stores.close();
    }
  ...
  await run("independent-copy-staging-and-cross-session-paste", async (name) => {
    ...
    try {
      ...
      const input = {
        copyId: "copy-one",
        sourceSessionId: "ps_fixture",
        sourceDraftId: source.result,
        sourceRevision: 1,
        type: sourceRecord.envelope.type,
        schemaVersion: 1,
        payload: sourceRecord.payload,
        media: sourceRecord.media
      };
      const pending = stores.copy.stage(0, input);
      input.payload.title = "mutated after capture";
      const staged = await pending;
      equal(staged.revision, 1);
      assert(staged.entry);
      equal(staged.entry.payload, { title: "fixture" });
      ...
      equal(await stores.blobs.getBlob("source-media"), void 0);
      equal([...(await stores.blobs.getBlob(copyBlob)).data], [42]);
      const pasted = await target.pasteCopy(0, 1);
      equal(pasted.current.revision, 1);
      ...
      await stores.copy.clear(1);
      equal(await stores.blobs.getBlob(copyBlob), void 0);
  ...
  await run("copy-cas-tombstones-and-stale-source", async (name) => {
    ...
    try {
      ...
      equal(outcomes.find((item) => item.status === "rejected").reason.code, "ATT_STALE_REVISION");
      const cleared = await b.copy.clear(1);
      equal(cleared.revision, 2);
  ...
  await run("copy-rollback-preserves-old-copy-and-bytes", async (name) => {
    ...
    try {
      ...
      const value = {
        ...
        schemaVersion: 1,
        payload: source.current.view.records[0].payload,
        media: source.current.view.records[0].media
      };
      const first = await stores.copy.stage(0, value);
      const retainedId = first.entry.media[0].draftResourceId;
      try {
        IDBObjectStore.prototype.put = function(...args) {
          if (this.name === ATTACHMENT_COPY_STORE_NAME) throw new DOMException("fixture write abort", "QuotaExceededError");
          return original.apply(this, args);
        };
      ...
      equal((await stores.copy.load()).entry.media[0].draftResourceId, retainedId);
      equal([...(await stores.blobs.getBlob(retainedId)).data], [1]);
      const scoped = await stores.blobs.listBlobs("ps_fixture", "pibo.copy-holder.replacement");
      equal(scoped, []);
    } finally {
      IDBObjectStore.prototype.put = original;
      draft.close();
  ...
  await run("copy-paste-rejects-stale-missing-and-foreign-owner", async (name) => {
    ...
    try {
      ...
      await rejects(foreignStores.copy.stage(0, value), "ATT_STALE_REVISION");
      equal((await foreignStores.copy.load()).revision, 0);
      await rejects(foreign.pasteCopy(0, 0), "ATT_STALE_REVISION");
      equal((await foreign.load()).revision, 0);
      const first = await stores.copy.stage(0, value);
      await rejects(target.pasteCopy(0, 0), "ATT_STALE_REVISION");
      equal((await target.load()).revision, 0);
      await stores.copy.clear(first.revision);
      await rejects(target.pasteCopy(0, first.revision), "ATT_STALE_REVISION");
      const next = await stores.copy.stage(2, value);
      assert(next.entry);
      await attachmentIdbTransaction(db, [ATTACHMENT_BLOB_STORE_NAME], "readwrite", async ([bytes]) => {
        await attachmentIdbRequest(bytes.delete(next.entry.media[0].draftResourceId));
      });
      await rejects(target.pasteCopy(0, next.revision), "ATT_BYTES_MISSING");
      equal((await target.load()).revision, 0);
      equal((await stores.copy.load()).revision, next.revision);
    } finally {
      draft.close();
      target.close();
      foreign.close();
  ...
  await run("copy-revision-exhaustion-preserves-row", async (name) => {
    ...
    try {
      ...
      await attachmentIdbTransaction(db, [ATTACHMENT_COPY_STORE_NAME], "readwrite", async ([copies]) => {
        await attachmentIdbRequest(copies.put(row));
      });
      await rejects(stores.copy.clear(Number.MAX_SAFE_INTEGER), "ATT_LIMIT_EXCEEDED");
      await rejects(stores.copy.stage(Number.MAX_SAFE_INTEGER, {
        copyId: "new",
        sourceSessionId: "ps_fixture",
        sourceDraftId: source.result,
        sourceRevision: 1,
      ...
      equal(await attachmentIdbTransaction(db, [ATTACHMENT_COPY_STORE_NAME], "readonly", async ([copies]) => attachmentIdbRequest(copies.get("owner"))), row);
    } finally {
      draft.close();
      stores.close();
      db.close();
  ...
  await run("two-real-tabs-one-copy-cas-winner", async (name) => {
    ...
    try {
      ...
      const remote = await callPeer("copyInit", { databaseName: name });
      assert(remote.ok);
      equal(remote.revision, 0);
      ...
      equal(outcomes.find((item) => !item.ok).code, "ATT_STALE_REVISION");
      equal((await stores.copy.load()).revision, 1);
    } finally {
  ...
  window.addEventListener("message", async (event) => {
    ...
    try {
      if (operation === "init") {
        ...
        result = { ok: true };
      } else if (operation === "copyInit") {
        stores = await openAttachmentStores(indexedDB, "owner", safeName(event.data.databaseName));
        result = { ok: true, revision: (await stores.copy.load()).revision };
      } else if (operation === "copyStage") {
        const staged = await stores.copy.stage(event.data.expectedRevision, event.data.value);
        result = { ok: true, revision: staged.revision };
