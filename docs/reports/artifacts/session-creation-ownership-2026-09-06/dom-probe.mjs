...
try{for(const width of [1431,390])for(const scenario of ['ordinary','untouched','session-before-refresh','session-after-refresh','other-room','browser-back','next-create']){
 ...
 await c.evaluate(`(()=>{
  ...
  const frame=()=>{if(state.done)return;state.frames.push(state.snapshot());requestAnimationFrame(frame)};requestAnimationFrame(frame);document.querySelector('button[aria-label="New Session"]').click();
 })()`);
 try{
  ...
  if(scenario==='next-create'){
   await c.evaluate(`document.querySelector('button[aria-label="New Session"]').click()`);await waitFor('window.__creationAcceptance.creations.length===2');
   ...
   if(scenario==='browser-back'){
    ...
    if(scenario==='other-room'){await c.evaluate(`document.querySelector('[data-pibo-debug="room-node"][data-pibo-room-id="${room}"] button').click()`);await waitFor(`!!document.querySelector('[data-pibo-debug="session-row"][data-pibo-session-id="${expected}"]')`);await sidebar();}
    await c.evaluate(`document.querySelector('[data-pibo-debug="session-row"][data-pibo-session-id="${expected}"] button[aria-label^="Open session"]').click()`);
   }
   await waitFor(`document.querySelector('[data-pibo-debug="compact-terminal-session-view"]')?.dataset.piboSessionId===${JSON.stringify(expected)}`);
  ...
  const data=await c.evaluate(`(()=>{const s=window.__creationAcceptance;s.done=true;return{at:new Date().toISOString(),frames:s.frames,requests:s.requests,creations:s.creations,heldAt:s.heldAt,intentAt:s.intentAt,releasedAt:s.releasedAt,final:s.snapshot(),scripts:[...document.scripts].filter(s=>s.src).map(s=>s.src)}})()`);
  ...
  if(!['ordinary','untouched','next-create'].includes(scenario)&&!data.final.visible.some(t=>t.includes('CREATION_TARGET_'+expected)))throw Error('Correct real assistant marker is not viewport-visible');
  if(width===390&&scenario==='browser-back'){const shot=await c.send('Page.captureScreenshot',{format:'png'});await writeFile('/tmp/creation-pibo2-mobile-back.png',Buffer.from(shot.data,'base64'));}
 }finally{await release();await c.evaluate('if(window.__creationAcceptance)window.__creationAcceptance.done=true');await writeFile('/tmp/creation-pibo2-acceptance.json',JSON.stringify({ids,results},null,2));}
}
