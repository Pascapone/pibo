...
**Overall result:** **BLOCKED pending focused Pibo2 readiness plus one human evidence gate.** The August 16 follow-up audit found and corrected a real provider-auth architecture gap: Chat Web had still written Pi `AuthStorage` before runtime routing. The runtime-neutral correction now passes local deterministic verification, but its exact Pibo2 candidate and focused PR are not yet recorded in this revision. Pibo2-managed native-Codex production-provider authentication will still require authorized interactive verification afterward.
...
- Focused runtime-auth candidate commit/package SHA-256: pending exact commit and package build
...
- Existing PR stack: #476, #477, #478, #479, #483, #486, #487, #488, #489, #490, #491, #492, #493, #494, #495, #497, #498, #499, #501, #502, and #503
- Focused runtime-auth PR: pending
...
| REQ-009 Agent Designer | catalog APIs, custom-agent store, Designer runtime controls, runtime-scoped auth join, and disabled explanations | Designer autosave/default-model/catalog/save validation plus `chat-ui-provider-auth-methods.test.mjs` | Focused correction and 1,752-test full suite | Prior authenticated browser rendered native runtime diagnostics/options/capabilities; focused Pibo2 auth-aware render pending | #479, #495, #498, #499, #501; focused auth PR pending | Exact focused browser evidence pending | LOCAL PASS / PIBO2 PENDING |
...
| REQ-017 Complete verification | reusable contract suites, import boundaries, auth fixtures, Pibo2 ladder | all files above plus `agent-runtime-auth`, `login-actions`, `codex-native-auth`, provider Web/UI, debug PTY, workflow/Loop/Cron tests | Typecheck/build passed; **1,752/1,752 tests across 12 suites**; package inventory passes | Prior integrated Pibo2 matrix passes; focused auth candidate and authenticated native turn remain | Entire stack plus focused auth PR pending | **Focused Pibo2 readiness and one authenticated native-Codex production-provider run are absent.** | BLOCKED |
| REQ-018 Reviewable delivery | canonical specs/plans/project docs/reports, focused branches/commits | documentation links and diff checks | focused local verification and clean diff pass | Prior package/report/assets recorded; focused exact candidate pending | #476–#502, #503; focused auth PR pending | Focused branch is not yet pushed/opened | PENDING |
| REQ-019 Runtime provider auth | `src/agent-runtime/auth.ts`, registry/router/channel dispatch, Pi/Codex auth controllers, product API, Provider Settings, Terminal/Designer model joins | `agent-runtime-auth`, `login-actions`, `codex-native-auth`, routed-session, Web, UI-source, skill tests | Typecheck/build and 1,752/1,752 full suite pass | Exact Pibo2 API/browser readiness pending | Focused auth PR pending | Native production-provider turn intentionally remains a later human gate | LOCAL PASS / PIBO2 PENDING |
...
| Runtime-neutral provider-auth correction | Focused PR pending; `runtime-auth-control-plane-validation-2026-08-16.md` |

## Final decision

The previous statement that only external interactive authentication remained was incomplete: the Chat Web-to-Pi auth bypass was a real architecture gap. That gap is now corrected locally and the 1,752-test canonical suite passes. Completion still requires (1) committing/pushing the focused reviewable PR, (2) activating and validating its exact package on Pibo2 while native Codex remains disconnected, and then (3) an authorized human completing managed login so one bounded public native-Codex production-provider turn can close REQ-015, REQ-017, and the overall goal.
