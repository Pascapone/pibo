...
title: "Design: Multi-Agent Runtime Adapters"
...
Task 9.6 adds a session-owned `CodexNativeRequestController` for stable command/file approvals and experimental structured user input. Command/file requests are advertised after exact protocol validation. Structured input remains disabled by default and is advertised only when the configured runtime instance explicitly enables `experimentalUserInput`; App Server initializes with `experimentalApi: false` except for a profile that explicitly selects Codex Plan permission mode. The controller validates exact thread/turn scope, replaces native request ids with opaque product ids, bounds and recursively redacts projected payloads, validates decisions/answers, and tracks all pending requests for status/API/UI. `serverRequest/resolved`, turn replacement/completion, interruption, process failure, and disposal cancel deferred handlers without a stale response. Generic Pibo actions and SSE/UI consume normalized request contracts without a Codex-specific orchestration branch. Exact `0.147.0` validation proves accepted command/file approvals, structured-input round trip, pending-approval interruption, child-process restart/resume, private permissions, unchanged global Codex state, and zero leaked App Server processes.

Task 9.7 implements native model, runtime-option, and permission-mode control without enabling experimental `thread/settings/update`. A bounded paginated `model/list` reader projects visible models, product-selectable reasoning efforts, native-only effort metadata, service tiers, modalities, defaults, and personality support into the runtime-scoped catalog. Model, reasoning effort, reasoning summary, service tier, personality, approval policy, and sandbox selection are sent through thread start/resume and turn start fields. Approval maps to `on-request` with workspace-write sandboxing, YOLO maps to `never` with danger-full-access sandboxing, and Plan maps to `on-request`, read-only sandboxing, and the experimental `collaborationMode: plan` turn field. Plan alone enables the initialize experimental API capability. Idle controls stage changes for the next turn; the adapter normalizes App Server's `default` service-tier sentinel to Pibo normal mode. Safe model/reasoning/tier/summary/personality selections are stored in binding metadata so a child-process restart preserves Pibo controls even when `0.147.0` reconstructs a resumed thread with model defaults. Cumulative `thread/tokenUsage/updated` notifications, including the stable resume replay, populate generic context usage. Agent Designer intersects runtime reasoning values with the selected model, and `/model` reads the active runtime catalog instead of always opening Pi's process-global catalog. Exact `0.147.0` validation proves the five-model catalog, priority and normal tiers, max-to-low reasoning changes, model switching, Approval/YOLO/Plan protocol mapping, Plan-only experimental initialization, cumulative usage replay, private boundaries, unchanged global Codex state, and zero leaked processes.

Task 9.8 delivers portable resources through stable App Server channels. Selected Pibo tools use the generation-scoped Streamable HTTP bridge and a bearer-token environment variable. Five-minute leases renew during active turns; new turns refresh an older process generation, and idle sessions hand off before the credential's 30-minute absolute maximum by stopping the owned App Server, issuing a fresh credential, and resuming the same durable thread. Fresh empty threads are restarted rather than falsely resumed because `0.147.0` has not persisted them yet. A failed handoff leaves an explicit safe status diagnostic and retry instead of a silently closed runtime.

Selected external MCP servers are protocol-verified by the resource service and reverified through stable `mcpServerStatus/list`. HTTP headers use scoped environment indirection. Stdio definitions run through a packaged generic launcher: the actual command, arguments, working directory, and child environment stay in adapter process environment, while Codex receives only the Node launcher plus generated variable names. Codex forwards only those names, and the launcher removes them before spawning the selected server, preventing cross-server credential leakage. Exact verified tool names become `enabled_tools`; unsupported transports fail before verifier execution.

Selected skills use private generation roots, stable `skills/extraRoots/set`, and `skills/list`; every selected path must be present and enabled. Explicit Pibo product/context contributions are bounded and sent as `developerInstructions`. Automatic workspace context remains Codex-native `AGENTS.md` discovery. Generated Pibo MCP CLI guidance is not injected when Codex receives native MCP inventory. The adapter never sets `baseInstructions`, loads the Pi prompt, or mutates global Codex configuration. Start, resume, fork, clone, credential handoff, and normal session restart all reapply and verify the same selected resource plan. Exact `0.147.0` validation proves Pibo MCP, external HTTP/stdio MCP, selected skill/context, native project context, active-turn lease renewal, idle process handoff, durable restart/resume, secret isolation, private permissions, unchanged global state, and zero leaked processes.

Task 9.9 separates native-tool preservation, inspection, and yielding. Exact stable v2 schema inspection finds no complete native-tool list request, so Codex declares `nativeToolInspection` as degraded `observed-runtime-items`; it does not infer a catalog from model names, experimental Apps metadata, or private implementation details. Runtime status exposes selected MCP tools immediately as stable `server/tool` names and adds harness-native names after scoped stable item notifications produce normalized tool-call events. Selected names are capped at 512 and observed names at 256; selected-inventory truncation produces an explicit warning. The observed set remains session-local, while selected resources are reconstructed and visible immediately after process or gateway resume.

This degraded product surface does not weaken preservation proof. Exact `0.147.0` validation records the provider-visible no-resource baseline, starts an otherwise equivalent selected-Pibo-MCP session, and compares the native namespace/tool structures. Fixed definitions and every baseline code-mode/native name remain present. Codex's dynamic `functions.exec` description legitimately expands with MCP resource helpers, and turn metadata adds the selected `mcp__pibo_session_tools__alpha` name; these are additive and no native entry disappears. Thread start/resume remain free of `baseInstructions` and native tool/config overrides. Pibo observes execution through existing command/file/MCP semantic events without wrapping or re-executing Codex-owned tools.

Task 9.10 routes Pibo-managed subagents through the existing portable tool and Pibo Session architecture rather than Codex's harness-native collaboration tools. A selected `pibo_subagent_*` definition reaches Codex through the generation-scoped Pibo MCP server. Because Codex `0.147.0` otherwise prompts and rejects model-initiated custom MCP calls when no MCP approval UI is active, only this generated, explicitly selected, credential-allowlisted Pibo server receives `default_tools_approval_mode = "approve"`. External MCP servers retain their native approval policy. Exact validation must execute the Pibo subagent from a model-generated `functions.exec` call, not only through `mcpServer/tool/call`.
