...
**Pibo Remote Agent** — The `pibo.remote-agent` plugin for room-scoped remote access: one loopback MCP endpoint serves all enabled rooms; per-room modules (sessions, observe, files, bash), Device-Code auth with 30-day revocable tokens, and a Sandbox/YOLO boundary. `remote_session_observe` shares the query engine with `pibo_agents_observe` with documented deviations (cursor scope per session plus query, roles superset, full-scan cost).
...
**Subagent / Delegated Agent** — A profile-selected agent that invokes another profile through a created or reused child Pibo Session. `pibo_agents_send_message` supports direct sends that wait for the delegated reply; when Run Control is available, `pibo_run_start` may wrap the send for asynchronous lifecycle control. List, observe, and kill remain direct management tools. The child freezes its target profile's runtime binding independently from the parent, so parent and child may use different adapters. A bounded thread key controls reuse. Normal parent-turn completion, a `pibo_run_wait` timeout, and stale telemetry do not stop active child work. Natural settlement, explicit run cancellation, agent kill, parent abort, or session/router disposal ends the active request without deleting a reusable child session unless the child is killed or disposed.
